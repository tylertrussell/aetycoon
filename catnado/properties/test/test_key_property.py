...
class KeyPropertyAssignmentTest(SimpleAppEngineTestCase):
  ...
  def setUp(self):
    ...
    self.one_sock = ImmutableKeyPropertyTestModel()
    self.one_sock_key = self.one_sock.put()

  def test_set_with_key(self):
    two_sock = ImmutableKeyPropertyTestModel(friend_key=self.one_sock_key)
    two_sock.put()
    self.assertEqual(two_sock.friend_key, self.one_sock_key)

  def test_set_with_entity(self):
    two_sock = ImmutableKeyPropertyTestModel(friend_key=self.one_sock)
    two_sock.put()
    self.assertEqual(two_sock.friend_key, self.one_sock_key)
